...
class EmailSender:
    ...
    def send_email_to_customer(self, recepient_email, message, confirmed, active_india, confirmed_india, recovered_india, deaths_india):
        try:
            ...
            body = message
            body = body.replace("$(confirmed)", confirmed)
            body = body.replace("$(active_india)", active_india)
            body = body.replace("$(confirmed_india)", confirmed_india)
            body = body.replace("$(recovered_india)", recovered_india)
            body = body.replace("$(deaths_india)", deaths_india)
    ...
    def send_email_to_support(self,cust_name,cust_email,cust_contact,confirmed,body):
            try:
                self.config_reader = ConfigReader()
                self.configuration = self.config_reader.read_config()
